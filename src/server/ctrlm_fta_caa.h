...
typedef int32_t (*ctrlms_audio_frame_t)[MIC_CHANNEL_QTY][SAMPLES_PER_FRAME];

typedef struct {
   bool   pass;
   double snr[MIC_CHANNEL_QTY];
} ctrlms_test_result_t;

#ifdef __cplusplus
extern "C" {
#endif

bool ctrlms_mic_test_audio_analyze(ctrlms_test_type_t test_type, const char *output_filename, uint32_t level, ctrlms_audio_frame_t audio_frames_noise, ctrlms_audio_frame_t audio_frames_signal, uint32_t frame_qty, uint32_t mic_qty, ctrlms_test_result_t *test_result);
