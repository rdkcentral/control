...
public:
     ...
     * @return True on success, otherwise False
     */
    bool append_config(const std::string &file_path);
    /**
     ...
     */
    json_t *json_from_path(const std::string &path, bool add_ref = false);
    /**
     ...
     */
    std::string string_from_path(const std::string &path);
