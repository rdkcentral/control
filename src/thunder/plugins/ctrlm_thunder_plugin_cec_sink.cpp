...
ctrlm_thunder_plugin_cec_sink_t *ctrlm_thunder_plugin_cec_sink_t::getInstance() {
    XLOGD_INFO("cec sink");
    static ctrlm_thunder_plugin_cec_sink_t instance;
    return &instance;
}
...
bool ctrlm_thunder_plugin_cec_sink_t::_update_cec_devices() {
    ...
    XLOGD_INFO("Calling CECSink for device data");
    ...
    if(this->call_plugin("getDeviceList", (void *)&params, (void *)&response)) {
        if(response.HasLabel("deviceList")) {
            ...
            XLOGD_ERROR("CECSink getDeviceList response malformed: <%s>", response_str.c_str());
        }
    } else {
        XLOGD_ERROR("CECSink getDeviceList call failed!");
    }
