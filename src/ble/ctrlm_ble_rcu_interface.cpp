        ...
        auto mfvDetectionTypeSlot = [this, address](BleRcuAudioService::DetectionType type)
            {
                ...
                params.rcu_data.ieee_address = address.toUInt64();
                params.rcu_data.mfv_detection_type = static_cast<ctrlm_hal_ble_MfvDetType_t>(type);
                m_rcuStatusChangedSlots.invoke(&params);
            };
...
ctrlm_hal_ble_rcu_data_t ctrlm_ble_rcu_interface_t::getAllDeviceProperties(uint64_t ieee_address)
{
    ...
    if (auto audioSvc = device->audioService()) {
        ...
        ret.mfv_detection_type = static_cast<ctrlm_hal_ble_MfvDetType_t>(audioSvc->mfvDetectionType());
