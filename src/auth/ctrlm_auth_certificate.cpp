...
ctrlm_auth_certificate_t::ctrlm_auth_certificate_t() {
   ...
   if(cert_locator == NULL){
      XLOGD_TELEMETRY("P12 cert selector init failed");
      ...
      XLOGD_TELEMETRY("P12 certificate <%s>:<%d>", cert_path, cert_status);
      if(cert_status != certselectorOk) {
         XLOGD_TELEMETRY("P12 cert selector retrieval failed");
         XLOGD_TELEMETRY("cert selector retrieval failed");
      } else {
         if(cert_path == NULL || cert_password == NULL) {
            XLOGD_TELEMETRY("P12 cert selector get failed");
            ...
            XLOGD_TELEMETRY("P12 cert selector local_path <%s>",local_path);
